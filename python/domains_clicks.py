...
# were recorded on each domain AND each subdomain under it. For example, 
...
#     retrieve the main domain domain and subdomain names
    
    for count in counts:
        count_numbers = count.rsplit(',',1)[0]
         # get main domain names
        main_domain_names = count.rsplit('.')[-1]
        subdomain_names = count.split('.',)[-2]

        print(main_domain_names, subdomain_names)
